...
def EfficientNet(inputs,classes,input_shape):
    return EfficientNetB4(classes=classes,input_shape=input_shape,input_tensor=inputs)
...
def train(FLAGS):
    ...
    with strategy.scope():
        ...
        if backbone == BACKBONE.MOBILENETV2:
            model = factory.build(mobilenetv2,0,alpha=1.4,classes=len(class_names))
            ...
            model = factory.build(EfficientNet, 0,classes=len(class_names),input_shape=(*input_size,3))
        model.compile(tf.keras.optimizers.Adam(1e-3), loss=tf.keras.losses.sparse_categorical_crossentropy,
    ...
    cos_lr = tf.keras.callbacks.LearningRateScheduler(
        lambda epoch, _: tf.train.cosine_decay(1e-3, epoch, epochs)().numpy(), 1)
    logging = tf.keras.callbacks.TensorBoard(log_dir=log_dir, write_images=True)
    checkpoint = tf.keras.callbacks.ModelCheckpoint(filepath=os.path.join(log_dir, 'ep{epoch:03d}-loss{loss:.3f}-val_loss{val_loss:.3f}.h5'), save_weights_only=True, verbose=1, period=3)
    model.fit(train_dataset, epochs=epochs, steps_per_epoch=max(1, train_num // batch_size),
                     validation_data=val_dataset, validation_steps=max(1, val_num // batch_size),
                     callbacks=[cos_lr, logging, checkpoint])
    model.save_weights(os.path.join(log_dir,str(backbone).split('.')[1].lower() + '_trained_weights.h5'))