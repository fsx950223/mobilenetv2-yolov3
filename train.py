...
def _main():
    ...
    if True:
        with strategy.scope():
            model.compile(optimizer=tf.keras.optimizers.Adam(1e-3,epsilon=1e-8),
                            loss=[lambda y_true, y_pred:yolo_loss(y_pred, y_true,input_shape,anchors,num_classes,print_loss=True)]*3,
                            )
    ...
    if True:
        ...
        model.fit(train_dataset,
                  epochs=20, initial_epoch=0, steps_per_epoch=max(1, train_num // batch_size),
...
def create_mobilenetv2_model(input_shape, anchors, num_classes, load_pretrained: bool = True,
    ...
    if freeze_body in [1, 2]:
        ...
        for i in range(num): model_body.layers[i].trainable = False
        print('Freeze the first {} layers of total {} layers.'.format(num, len(model_body.layers)))
    y_data = tf.keras.layers.Lambda(lambda output:[yolo_head(output[l], anchors[anchor_mask[l]], num_classes, input_shape, True)[1] for
              l in range(num_anchors // 3)])(model_body.output)
    model = tf.keras.models.Model(model_body.input, y_data)
    return model
