...
def EfficientNet(input_shape, block_args_list, global_params, include_top=True, pooling=None,input_tensor=None):
    ...
    if include_top:
        ...
        x = tf.keras.layers.Dense(global_params.num_classes, kernel_initializer=dense_kernel_initializer)(x)
        x = tf.keras.layers.Softmax()(x)
    else:
        if pooling == 'avg':
            x = tf.keras.layers.GlobalAveragePooling2D(data_format=global_params.data_format)(x)
        elif pooling == 'max':
            x = tf.keras.layers.GlobalMaxPooling2D(data_format=global_params.data_format)(x)
