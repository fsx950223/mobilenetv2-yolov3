...
class Dataset(tf.keras.callbacks.Callback):
    ...
    def tfrecord_dataset(self,files: List[str]):
        with tf.device('/cpu:0'):
            ...
            def parse(example_proto):
                ...
                image, bbox = get_random_data(image, xmins, xmaxs, ymins, ymaxs, labels, self.input_shape, train=self.train)
            ...
            if self.train:
                ...
                dataset = dataset.interleave(
                    ...
                    cycle_length=len(files),num_parallel_calls=AUTOTUNE).shuffle(train_num).map(parse, num_parallel_calls=AUTOTUNE).prefetch(self.batch_size).batch(self.batch_size).repeat()
            else:
                dataset = dataset.interleave(
                    lambda file: tf.data.TFRecordDataset(file),
                    cycle_length=len(files),num_parallel_calls=AUTOTUNE).map(parse, num_parallel_calls=AUTOTUNE).prefetch(self.batch_size).batch(self.batch_size).repeat()
            return dataset

    def text_dataset(self,files: List[str]):
        with tf.device('/cpu:0'):
            dataset = tf.data.Dataset.from_tensor_slices(files)
            def parse(line):
                ...
                image, bbox = get_random_data(image,xmin,xmax,ymin,ymax,label,self.input_shape, train=self.train)
                y1, y2, y3 = tf.py_function(preprocess_true_boxes, [bbox, self.input_shape, self.anchors, self.num_classes],
                                            [tf.float32, tf.float32, tf.float32])
                return image, (y1, y2, y3)
            ...
            if self.train:
                ...
                dataset = dataset.interleave(
                    ...
                    cycle_length=len(files),num_parallel_calls=AUTOTUNE).shuffle(train_sum).map(parse, num_parallel_calls=AUTOTUNE).batch(self.batch_size).prefetch(self.batch_size).repeat()
            else:
                dataset = dataset.interleave(
                    lambda file: tf.data.TextLineDataset(file),
                    cycle_length=len(files),num_parallel_calls=AUTOTUNE).map(parse, num_parallel_calls=AUTOTUNE).batch(self.batch_size).prefetch(self.batch_size).repeat()
            return dataset

    def __init__(self,glob_path:str,batch_size,input_shapes,anchors,num_classes,train: bool = True):
        self.glob_path=glob_path
        self.batch_size=batch_size
        if isinstance(input_shapes,list):
            self.input_shapes = input_shapes
            self.input_shape = tf.get_variable("input_shape",initializer=self.input_shapes[0],trainable=False)
        else:
    ...
    def build(self):
        ...
        try:
            num = reduce(lambda x, y: x + y, map(lambda file: int(file.split('/')[-1].split('.')[0].split('_')[-1]), files))
        except Exception:
            print('Please format file name like <name>_<number>.<extension>')
        else:
